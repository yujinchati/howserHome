	...
	var touchSlider = {
		...
		resize : function (e) {
			if(e.opts.flexible) {
				...
				var temp_h = e._tg.children().children().children().height();
				e._list.css({
					"width" : e._item_w + "px",
				});
				e._list.parent().css({
		...
		counter : function () {
			...
			if(this.opts.resize) {
				this._tg.css({
					...
					"margin-top" : ("-"+this._list.eq(this.opts.page-1).height()/2) + "px",
				});
			}
			this._list.eq(this.opts.page-1).addClass('on').siblings().removeClass('on');
			if(this.opts.paging) {
				this._pagingBtn.eq(this.opts.page - 1).addClass('ts-paging-active').siblings().removeClass('ts-paging-active');
			}
